...
public class RastaTraderManager implements SpecialSpawner {
    ...
    private boolean trySpawn(ServerWorld world) {
        ...
        if (playerEntity == null) {
            ...
            if (blockPos3 != null && this.doesNotSuffocateAt(world, blockPos3)) {
                ...
                RastaEntity RastaEntity = (RastaEntity) ModEntities.RASTA.spawn(world, blockPos3, SpawnReason.EVENT);
                Rootkz.LOGGER.info("SPAWNEO UNO!");
                if (RastaEntity != null) {
                    for(int j = 0; j < 2; ++j) {
                        this.spawnLlama(world, RastaEntity, 4);
                    }
    ...
    private void spawnLlama(ServerWorld world, RastaEntity wanderingTrader, int range) {
